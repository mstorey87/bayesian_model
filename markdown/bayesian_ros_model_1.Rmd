...
Even though this is a minimal model, we can compile it and derive posterior samples of the  parameters...
...
This is only defining prior beliefs about wind speed at the moment. ie. wind speeds are 0 to 20, and come from a normal distribution.
No observed data, so we will only get samples of the prior.
Next step (not yet) is to infer the underlying windspeed distribution based on observed wind speed measurements.
...
plot our prior belief about the distrubution of windspeed. No data observed yet to influence 

```{r}
post2_draws <- as_draws_df(post2$draws())
mu <- mean(post2_draws$windspeed) |> round(2)
q90 <- quantile(post2_draws$windspeed, c(0.05, 0.95)) |> round(2)

plot(density(post2_draws$windspeed, from=0, adj = 1), 
...
```
what is our estimate of true population distribution of mean wind speed and sigma parameter of normal distribution of windspeed.
```{r}
post3_draws <- as_draws_df(post3$draws())
mu <- round(mean(post3_draws$avg_windspeed), digits = 2)
q <- round(quantile(post3_draws$avg_windspeed, c(0.05, 0.95)), digits = 2)

dat_gg <- as.data.frame(post3_draws) %>%
...
Having a shared sd could be because You believe that while wind speed averages vary by line, the  fluctuation around those averages is similar everywhere (e.g., due to shared atmospheric conditions).
...
parameters {
  ...
  // ie assuming the variability (spread) of windspeed is similar across all locations
  real<lower=0> sigma;
}
model {
  // Prior for the common standard deviation with a mean of 5.0 (1/0.2)
...
```{r}
post4_draws <- as_draws_df(post4$draws())
dat_gg <- as.data.frame(post4_draws) %>% 
  select(-lp__) %>%
...
```{r}
post4a_draws <- as_draws_df(post4a$draws())
dat_gg <- as.data.frame(post4a_draws) %>% 
  select(-lp__) %>%
...
data {
  ...
  array[W] int<lower=1> wline_id;  // line ID for each weather observation (id matching ros id)
...
parameters { // unknowns or unobserved parameters
...
post5_draws <- as_draws_df(post5$draws())
```
...
dat_post5 <- as.data.frame(post5_draws)

# Wind and RH values for predictions
dat_pred <- expand.grid(
  wind = c(0,10, 20, 30, 40,50,60,70),
  rh = c(10,20,30,40,50,60,70, 80) / 100
)
...
# df <- data.frame(rh = ..., wind = ..., ros = ...)

ggplot(ros_pred, aes(x = rh*100, y = wind, fill = mid)) +
  geom_tile() +
  scale_fill_viridis_c(option = "plasma") +  # or "magma", "viridis", etc.
  labs(
    title = "Mid Rate of Spread by RH and Wind Speed",
    x = "Relative Humidity",
    y = "Wind Speed (km/h)",  # or whatever unit you're using
    fill = "ROS"
  ) +
  theme_minimal()
...
ui <- fluidPage(
  titlePanel("Predicted Fire Rate of Spread"),
...
server <- function(input, output) {
  output$rosPlot <- renderPlot({
    ...
    ros_pred <- dat_post5 %>%
      dplyr::cross_join(dat_pred) %>%
      mutate(
        mu = alpha + beta_wind * wind + beta_rh * rh,
        ros_log = rnorm(n(), mu, sigma_ros),
        ros = exp(ros_log)
      )
    ...
    ggplot(ros_pred, aes(x = ros)) +
      ...
      labs(
        ...
        x = "Rate of Spread (ROS)",
        y = "Density"
      ) +
      coord_cartesian(xlim = c(0, 20), ylim = c(0, 1)) +  # ← Adjust these as needed
      theme_minimal()
  })
}
...
data {
  ...
  vector<lower=0>[W] wind;                // wind speed
  vector<lower=0>[W] rh;                  // RH (as proportion 0-1)
  vector<lower=0>[L] ros;                 // rate of spread (positive)
}

parameters {
  vector[L] avg_wind;                     // imputed wind per line
  real<lower=0> sigma_wind;

  vector<lower=0.01, upper=0.9>[L] avg_rh; // imputed RH per line
  real<lower=0, upper=50> phi_rh;
  ...
  real<lower=0> shape_ros;                // shape parameter for gamma
}

model {
  vector[L] mu;     // mean ROS per line
  vector[L] rate;   // gamma rate = shape / mean

  // Weather model (same as before)
  for (i in 1:W) {
    wind[i] ~ normal(avg_wind[wline_id[i]], sigma_wind);
    rh[i] ~ beta_proportion(avg_rh[wline_id[i]], phi_rh + 1);
  }

  // Gamma likelihood for ROS
  for (i in 1:L) {
    mu[i] = exp(alpha + beta_wind * avg_wind[i] + beta_rh * avg_rh[i]);  // ensure positivity
    rate[i] = shape_ros / mu[i];
    ros[i] ~ gamma(shape_ros, rate[i]);
  }
}
"
```
...
post6 <- get_samples(
  model_code_6,
  data = model5_data,
  vars = c("alpha", "beta_wind", "beta_rh", "shape_ros"),
  refresh = 100
)
post6_draws <- as_draws_df(post6$draws())
```
...
ros_pred <- post6_draws %>%
  # Add prediction data to every posterior record
  cross_join(dat_pred) %>%
  
  # Predict ROS using Gamma distribution
  mutate(mu = exp(alpha + beta_wind * wind + beta_rh * rh),
...
ros_pred_sum <- ros_pred %>% 
  group_by(wind) %>% 
  summarise(ros_mid=median(ros))


ggplot(ros_pred_sum)+
  geom_line(aes(y=ros_mid,x=wind))
```
```{r}
model_code_7 <- "
data {
  ...
  vector<lower=0>[W] rh;
  vector<lower=0>[L] ros;  // observed ROS (must be positive)
}
...
parameters {
  ...
  vector<lower=0.01, upper=0.9>[L] avg_rh;
  real<lower=0, upper=50> phi_rh;
...
transformed parameters {
  vector[L] mu;    // expected ROS (bounded between 0 and max_ros)
  vector[L] rate;

  real max_ros = 10;  // maximum plausible ROS in km/h

  for (i in 1:L) {
    real linpred = alpha + beta_wind * avg_wind[i] + beta_rh * avg_rh[i];
    mu[i] = max_ros / (1 + exp(-linpred));  // sigmoid function
    rate[i] = shape_ros / mu[i];
  }
}
...
model {
  ...
  // ROS model with sigmoid-shaped predictor response
...
```{r}
post7_draws <- as_draws_df(post7$draws())


dat_pred <- expand.grid(
  wind = c(10, 20, 30, 40, 50, 60,70,80,90),
  rh = 0.9  # Relative humidity in proportion (0–1)
)

library(dplyr)
library(tidyr)

ros_pred <- post7_draws %>%
  crossing(dat_pred) %>%
  mutate(
    linpred = alpha + beta_wind * wind + beta_rh * rh,
    mu = 10 / (1 + exp(-linpred)),  # max_ros = 10 (km/h)
    ros = rgamma(n(), shape = shape_ros, rate = shape_ros / mu)
  )
...
ggplot(ros_pred) +
  geom_density(aes(x = ros), fill = "forestgreen", alpha = 0.5) +
  facet_wrap(~wind, ncol = 1, scales = "free_y") +
  labs(
    title = "Posterior Predictive Densities (Sigmoid ROS Model)",
    x = "ROS (km/h)",
    y = "Density"
  )
...
ros_pred_sum <- ros_pred %>%
  ...
  summarise(
    ...
    ros_lower = quantile(ros, 0.1),
    ros_upper = quantile(ros, 0.9),
    .groups = "drop"
  )

ggplot(ros_pred_sum) +
  geom_ribbon(aes(x = wind, ymin = ros_lower, ymax = ros_upper), fill = "lightgreen", alpha = 0.4) +
  geom_line(aes(x = wind, y = ros_median), color = "darkgreen", linewidth = 1.2) +
  geom_point(aes(x = wind, y = ros_median), size = 2) +
  labs(
    title = "Predicted ROS vs Wind Speed (Sigmoid Model)",
    x = "Wind speed (km/h)",
    y = "ROS (km/h)"
  )
