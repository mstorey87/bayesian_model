...
data {
  ...
  real<lower=0> PriorShape;     // mean for prior on Gamma shape: exponential(1.0/PriorShape) 
}
parameters {
  vector[Nv] beta;
...
model {
  ...
  shape ~ exponential(1.0 / PriorShape);
}
generated quantities {
  vector[Nr] lp;
  vector[Nr] ros_mean;
  array[Nr] real ros_obs;

  // predicted mean ROS
  lp = X*beta;
  ros_mean = MaxROS / (1 + exp(-lp));
  
  // predicted ROS observations
  ros_obs = gamma_rng(shape, shape / ros_mean);
}
"
...
prior_sim <- fit %>%
  tidybayes::spread_draws(shape, ros_mean[i], ros_obs[i], ndraws = Ndraws) %>%
  ungroup() %>%
